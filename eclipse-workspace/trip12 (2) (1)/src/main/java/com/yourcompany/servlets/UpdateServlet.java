...
@WebServlet("/UpdateServlet") // 서블릿 매핑을 어노테이션을 통해 정의
public class UpdateServlet extends HttpServlet { // HttpServlet 클래스를 상속받는 UpdateServlet 클래스 선언
    private static final long serialVersionUID = 1L; // 직렬화에 사용될 serialVersionUID 정의

    public UpdateServlet() { // 기본 생성자 정의
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException { // HTTP POST 요청을 처리하는 메소드
        try {
            ...
            // SQL 쿼리 준비
            String query = "UPDATE board SET title=?, content=? WHERE id=?"; // 게시글의 title과 content를 수정하는 SQL 쿼리문
            ...
            if (result > 0) { // SQL 쿼리 실행 결과가 0보다 큰 경우(수정 성공 시)
                response.sendRedirect("view.jsp?id=" + id); // 해당 게시글로 리다이렉트
            } else { // SQL 쿼리 실행 결과가 0인 경우(수정 실패 시)
                response.sendRedirect("edit.jsp?id=" + id); // 수정 페이지로 다시 리다이렉트
            }
