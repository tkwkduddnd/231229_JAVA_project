...
import java.util.ArrayList; // ArrayList 클래스를 위한 라이브러리
import java.util.List; // List 클래스를 위한 라이브러리
...
@WebServlet("/HoneymoonServlet") // 서블릿 매핑을 위한 어노테이션, 클라이언트가 "/HoneymoonServlet" 요청을 하면 이 서블릿이 처리
public class HoneymoonServlet extends HttpServlet { // HttpServlet을 상속받는 HoneymoonServlet 클래스 선언
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) // HTTP POST 요청을 처리하는 메소드
        ...
        try {
            ...
            StringBuilder queryBuilder = new StringBuilder("SELECT * FROM products WHERE theme=? AND theme='허니문'"); // SQL 쿼리를 생성하는 StringBuilder 객체 생성
            if (keyword != null && !keyword.isEmpty()) { // keyword가 null이 아니고 빈 문자열이 아니면
                queryBuilder.append(" AND productName LIKE ?"); // SQL 쿼리에 productName 조건 추가
            }

            String query = queryBuilder.toString(); // SQL 쿼리를 문자열로 변환
            ...
            while (rs.next()) { // ResultSet 객체에 다음 행이 있으면
                ...
                product.setId(rs.getInt("id")); // Product 객체에 id 설정
                product.setTheme(rs.getString("theme")); // Product 객체에 theme 설정
                product.setProductImage(rs.getString("productImage")); // Product 객체에 productImage 설정
                product.setProductCode(rs.getString("productCode")); // Product 객체에 productCode 설정
                product.setProductName(rs.getString("productName")); // Product 객체에 productName 설정
                product.setDepartureTime(rs.getString("departureTime")); // Product 객체에 departureTime 설정
                product.setArrivalTime(rs.getString("arrivalTime")); // Product 객체에 arrivalTime 설정
                product.setRemainingCount(rs.getInt("remainingCount")); // Product 객체에 remainingCount 설정
                product.setProductStatus(rs.getString("productStatus")); // Product 객체에 productStatus 설정
                product.setPrice(rs.getInt("price")); // Product 객체에 price 설정
                product.setDetails(rs.getString("details")); // Product 객체에 details 설정

                productList.add(product); // ArrayList에 Product 객체 추가
            }

            // JSP 페이지로 전달
            request.setAttribute("productList", productList); // 요청에 productList 속성을 설정
        ...
        // Honeymoon.jsp로 포워딩
        request.getRequestDispatcher("Honeymoon.jsp").forward(request, response); // 요청을 Honeymoon.jsp 페이지로 포워딩
    }
}
