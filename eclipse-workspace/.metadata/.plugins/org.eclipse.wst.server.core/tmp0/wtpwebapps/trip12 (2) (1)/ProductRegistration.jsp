...
<body> <!-- body 태그 시작 -->
    ...
    <section> <!-- section 태그 시작 -->
        <div class="admin-content"> <!-- admin-content 클래스 div 시작 -->
            ...
            <form action="ProductRegistrationServlet" method="post" enctype="multipart/form-data"> <!-- 폼 시작, 제출 시 ProductRegistrationServlet으로 POST 요청 보냄 -->
                <label for="theme">테마:</label> <!-- 라벨 설정 -->
                <select name="theme"> <!-- 테마 선택 -->
                    <option value="유럽">유럽</option> <!-- 유럽 테마 옵션 설정 -->
                    <option value="동남아">동남아</option> <!-- 동남아 테마 옵션 설정 -->
                    <option value="일본">일본</option> <!-- 일본 테마 옵션 설정 -->
                    <option value="중국">중국</option> <!-- 중국 테마 옵션 설정 -->
                    <option value="미국">미국</option> <!-- 미국 테마 옵션 설정 -->
                    <option value="허니문">허니문</option> <!-- 허니문 테마 옵션 설정 -->
                </select><br> <!-- 테마 선택 끝 -->

                <label for="productImage">상품 이미지:</label> <!-- 라벨 설정 -->
                <input type="file" name="productImageFile"><br> <!-- 파일 입력 필드 -->

                <label for="productCode">상품 코드:</label> <!-- 라벨 설정 -->
                <input type="text" name="productCode"><br> <!-- 상품 코드 입력 필드 -->

                <label for="productName">상품 이름:</label> <!-- 라벨 설정 -->
                <input type="text" name="productName"><br> <!-- 상품 이름 입력 필드 -->

                <label for="departureTime">출발일시:</label> <!-- 라벨 설정 -->
                <input type="datetime-local" name="departureTime"><br> <!-- 출발일시 입력 필드 -->

                <label for="arrivalTime">도착일시:</label> <!-- 라벨 설정 -->
                <input type="datetime-local" name="arrivalTime"><br> <!-- 도착일시 입력 필드 -->

                <label for="remainingCount">남은 갯수:</label> <!-- 라벨 설정 -->
                <input type="number" name="remainingCount"><br> <!-- 남은 갯수 입력 필드 -->

                <label for="productStatus">상품 상태:</label> <!-- 라벨 설정 -->
                <select name="productStatus"> <!-- 상품 상태 선택 -->
                    <option value="판매중">판매중</option> <!-- 판매중 옵션 설정 -->
                    <option value="품절">품절</option> <!-- 품절 옵션 설정 -->
                </select><br> <!-- 상품 상태 선택 끝 -->

                <label for="price">가격:</label> <!-- 라벨 설정 -->
                <input type="number" name="price"><br> <!-- 가격 입력 필드 -->

                <label for="details">상세 정보:</label> <!-- 라벨 설정 -->
                <textarea name="details"></textarea><br> <!-- 상세 정보 입력 필드 -->

                <input type="submit" value="등록"> <!-- 제출 버튼 -->
                <input type="button" value="취소" onclick="location.href='ProductRegistration.jsp'"> <!-- 취소 버튼, 클릭 시 ProductRegistration.jsp로 이동 -->
                <br>
            </form> <!-- 폼 끝 -->
